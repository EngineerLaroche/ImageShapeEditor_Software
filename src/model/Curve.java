...
import curve.PolylineCurveType;
...
public class Curve extends ShapeContainer implements ColoredShapeIF {
    ...
    public void recomputeLineSegments() {
    	...
    	lineSegments.clear();
    	for (int i = 0; i < curveType.getNbOfSegments(getShapes().size()); ++i){
    ...
    public List computeLineSections(int segmentNumber){
    	...
    	double deltaT = (curveType.getMaxT(segmentNumber)-curveType.getMinT(segmentNumber)) /
    	                numberOfSections;
    	for (int i = 0; i < curveType.getNbOfCtrlPointsPerSegment(); ++i){
    		currentControlPoints.set(i, curveType.getCtrlPoint(transformedControlPoints, segmentNumber, i));
    	}
	...
	private void resetCurrentControlPoints() {
		currentControlPoints = new ArrayList(curveType.getNbOfCtrlPointsPerSegment());
